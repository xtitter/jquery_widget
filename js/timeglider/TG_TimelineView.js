  ...
  tg.TG_TimelineView = function (widget, mediator) {
	...
	/* references specific to the instance (rather than timeglider) so
	   ...
	   this._views = {
			...
			ADD_COMMENT_BT : PL + " .full_modal_add_comment_button",
			ADD_COMMENT_FORM : PL + " .full_modal_add_comment"
	    }
  ...
  this._templates = {};
	...
	this._templates = {
      ...
      event_modal: this.getEventModalTemplate(),
		...
		event_modal_comment: this.getEventModalCommentTemplate(),
...
tg.TG_TimelineView.prototype = {
   ...
   getEventModalCommentTemplate : function() {
      // Nate just copied this from above.  All fricken templates
		// should be in external files, not defined in js!
      var me = this,
          template = false,
          stripped = '',
          default_template = $.template( null, "<div class='full_modal_comment'>"
            + "<div class='full_modal_comment_author'>Author: ${author}</div>"
            + "<div class='full_modal_comment_timestamp'>Time: ${timestamp}</div>"
            + "<div class='full_modal_comment_body'>${body}</div>"
            + "</div>");
          
      // DEFAULT TEMPLATE
      if (!options.event_modal_comment) {
       template = default_template;
      } else {
        // we have a custom event modal comment waiting in a file === get it
       var tget = $.ajax({
         url: options.event_modal_comment,
         success: function (html) { 
           // sets template to template in case it returns before the get happens
           stripped = html.removeWhitespace();
           debug.log("stripped:" + stripped);
           me._templates.event_modal_comment = template = $.template(null, stripped);
         },
         error : function () {
           me._templates.event_modal_comment = template = default_template;
           debug.log("Custom event modal HTML file could not be found/loaded! Reverting to default.");
         }
       });
  ...
  */
  ...
  // el charo nate -- this gets called each time the modal dialog gets
  //  set up - when an event is clicked.  oh well.
  setupAddComments : function(actual_eid) {
  	   var me=this;
		//console.log("add comment  " + me._views.ADD_COMMENT_BT);
		$(me._views.ADD_COMMENT_BT).click(function(){
			console.log("in add comment button click");
			$(me._views.ADD_COMMENT_BT).toggle('fast');
			$(me._views.ADD_COMMENT_FORM).toggle('slow');
		});
	   $(me._views.ADD_COMMENT_FORM + " .full_modal_add_comment_cancel").click(function(){
         $(me._views.ADD_COMMENT_FORM).toggle('fast');
         $(me._views.ADD_COMMENT_BT).toggle('slow');
      });
		$(me._views.ADD_COMMENT_FORM + " .full_modal_add_comment_submit").click(function() {
			// do error checking, hey
			console.log($(me._views.ADD_COMMENT_FORM + " .full_modal_add_comment_author_input"));
			var auth=$(me._views.ADD_COMMENT_FORM + " .full_modal_add_comment_author_input").text();
			var body=$(me._views.ADD_COMMENT_FORM + " .full_modal_add_comment_body_input").text();
			$.ajax({
         url: "db/add_comment.php?eid="+actual_eid+"&auth="+auth+"&body="+body,
	...
	eventModal : function (eid) {
		...
		var me = this,
		  ...
		  templ_obj = { 
			  ...
			  // el nate addito
			  ise_category: ev.category,
			  ise_record_type: ev.record_type,
			  ise_contributor: ev.contributor,
			  ise_creator: ev.creator,
			  ise_nsf_award_number: ev.nsf_award_number,
			  ise_pi: ev.pi,
			  ise_funding_source: ev.funding_source,
			  ise_uri: ev.uri,
			  ise_geo_coords: ev.geo_coords,
			  ise_access_inclusion: ev.access_inclusion,
			  ise_zipcode: ev.zipcode
  		}
       ...
       $modal
		  ...
		  // el nate addito el commento del las cerveza por favor
		  var actualId = eid.substr(3);
         $.ajax({
				dataType: "json", 
				url: "db/get_comments.php?event_id=" + actualId,
				success: function(data) {
					$(".full_modal_comment_number").html(data.length);
               var commentTemplate = me._templates.event_modal_comment;
               // might want to fix each data.timestamp here, although probably
               //  easier in get_comments.php ...
               $.tmpl(commentTemplate, data).appendTo(".full_modal_comments");
					console.log(me);
					me.setupAddComments(actualId);
				},
				error: function() {
					debug.log("Whoops -- error getting comments for event " + actualId);
				}
			});
